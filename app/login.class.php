...
class login{
	...
	function __construct()
	{
		...
		$_SESSION['libs'] = parse_ini_file("$this->iniDir/settings.ini");
		
		
		$_SESSION['libs']['item_id'] = 0;
		$_SESSION['libs']['user_email'] = '';
		$_SESSION['libs']['session_id'] = '';
		$_SESSION['libs']['is_admin'] = FALSE;
		$_SESSION['libs']['server_name'] = $_SERVER['HTTP_HOST'];
		
		//$_SESSION['libs'] = parse_ini_file("$this->iniDir/database.ini");
		$this->db = new db(new mysqli($_SESSION['libs']['server'],$_SESSION['libs']['user'], $_SESSION['libs']['password'],$_SESSION['libs']['db']));
		...
		//$_SESSION['libs']['web_data'] = 	$this->LABELS['web_data'];
		//$_SESSION['libs']['footer'] = 	$this->LABELS['footer'];
	}
	...
	private function changePasswdUser_fnc($id,$data)
	{
		...
		else
		{
			...
			if ($res['status'])
			{
				...
				$_SESSION['libs']['user_id'] = $result['user_id'];
				$_SESSION['libs']['user_email'] = $result['email'];
				$_SESSION['libs']['session_id'] = session_id();
				
				if ($result['account'] === 'admin')
				{
					$_SESSION['libs']['is_admin'] = TRUE;
				}
	...
	private function login_fnc($id,$data)
	{
		if (filter_var($data['email'], FILTER_VALIDATE_EMAIL))
		{
			...
			if (intval($res['id']) > 0)
			{
				$_SESSION['libs']['user_id'] = $res['id'];
				$_SESSION['libs']['user_email'] = $res['email'];
				$_SESSION['libs']['session_id'] = session_id();
	...
	private function registerNewUser_fnc($id,$data)
	{
		...
		if (filter_var($data['email'], FILTER_VALIDATE_EMAIL) == true)
		{
			...
			{
				$passwd1 = hash('md5',$data['password'].$_SESSION['libs']['salt']);
				$passwd2 = hash('md5',$data['password2'].$_SESSION['libs']['salt']);
				...
				if ($passwd1 === $passwd2) 
				{
					...
					if ($res['status'])	
					{
						...
						$_SESSION['libs']['user_id'] = $res['last_id'];
						$_SESSION['libs']['user_email'] = $data['email'];
						$_SESSION['libs']['session_id'] = session_id();
	...
	private function sendMailMsg($data)
	{
		...
		$this->mail->Host = $_SESSION['libs']['mail_server'];  	// Specify main and backup server
		$this->mail->Port = 25;
		$this->mail->SMTPAuth = true;                               // Enable SMTP authentication
		$this->mail->Username = $_SESSION['libs']['mail_acc'];                            // SMTP username
		$this->mail->Password = $_SESSION['libs']['mail_passwd'];                           // SMTP password
		//$this->mail->SMTPSecure = 'tls';                            // Enable encryption, 'ssl' also accepted
		
		$this->mail->From = $_SESSION['libs']['send_email_acc'];
		$this->mail->FromName = $_SESSION['libs']['mail_from_name'];
		//$this->mail->addAddress('josh@example.net', 'Josh Adams');  // Add a recipient
		$this->mail->addAddress($data['email']);               // Name is optional
		$this->mail->addReplyTo($_SESSION['libs']['mail_acc'], $_SESSION['libs']['mail_from_name']);
	...
	{
		$passwd = hash('md5', $data['password'].$_SESSION['libs']['salt']);
	...
	{	
		...
		$this->mail->From = $_SESSION['libs']['mail_acc'];
		$this->mail->FromName = $_SESSION['libs']['mail_from_name'];
		//$this->mail->addAddress('josh@example.net', 'Josh Adams');  // Add a recipient
		$this->mail->addAddress($email);               // Name is optional
		$this->mail->addReplyTo($_SESSION['libs']['mail_acc'], $_SESSION['libs']['mail_from_name']);
		$this->mail->addCC($_SESSION['libs']['mail_acc']);
	//	$this->mail->addBCC('bcc@example.com');
